...
const CreateEditProgramQuestionModal = ({
  ...
  useEffect(() => {
    if (editData) {
      setForm({
        ...
        radio_program_ids: editData?.radio_programs?.map((p) => p.id) || [],
  ...
  const handleProgramToggle = (id) => {
    setForm((prev) => ({
      ...prev,
      radio_program_ids: prev.radio_program_ids.includes(id)
        ? prev.radio_program_ids.filter((p) => p !== id)
        : [...prev.radio_program_ids, id],
    }));
  };
  ...
  const validateForm = () => {
    ...
    if (!form.radio_program_ids.length) {
      newErrors.radio_program_ids = "Please select at least one radio program";
      toast.error("select atleast one radio program");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        status: form.status,
        radio_program_ids: form.radio_program_ids,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 ">
      <div className=" w-full  bg-white rounded-2xl shadow-2xl overflow-auto max-h-[95vh]">
        {/* Sticky Header */}
        <div className="sticky top-0 z-20 bg-white px-6 pt-6  pb-4 border-b border-gray-200">
          {/* Close */}
          <button
            onClick={onClose}
            className="absolute top-4 right-6 sm:right-10 text-gray-400 hover:text-gray-700 hover:bg-gray-100 p-2 rounded-lg transition-colors"
          >
            <X size={24} />
          </button>
        ...
        {/* Content */}
        <div className="px-6 sm:px-10 py-6 overflow-auto">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                <p className="text-sm text-blue-700">
                  💡 <span className="font-medium">Tip:</span> Keep options
                  short and unique
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {programs.map((prog) => {
                  const selected = form.radio_program_ids.includes(prog.id);
                  return (
                    <button
                      key={prog.id}
                      type="button"
                    ...
                    >
                      <span className="truncate">
                        {prog?.program_category?.category}
                      </span>
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  ...
                  checked={
                    programs.length > 0 &&
                    form.radio_program_ids.length === programs.length
                  }
                  onChange={(e) =>
                    setForm((prev) => ({
                      ...prev,
                      radio_program_ids: e.target.checked
                        ? programs.map((p) => p.id)
                        : [],
                    }))
                  }
